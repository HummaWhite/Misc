...
inline int generatePrimes(int16_t *primes, int n)
{
	...
	int size = (n - 1) >> 1;
	uint8_t *isPrime = new uint8_t[size];
...
int main (int argc, char *argv[])
{
	...
	if (argc != 3)
	{
		if (procId == 0)
		{
			std::cout << "Command line: " << argv[0] << " <m>\n";
		}
		MPI_Finalize();
		return 0;
	}

	int maxNum = std::stoi(argv[1]);
	int sqrtNum = static_cast<int>(std::sqrt(static_cast<float>(maxNum)));

	int lVal = static_cast<int64_t>(procId) * (maxNum - sqrtNum - 1) / procNum + sqrtNum + 2;
	int rVal = static_cast<int64_t>(procId + 1) * (maxNum - sqrtNum - 1) / procNum + sqrtNum + 1;
	...
	uint8_t *marked = new uint8_t[markTableSize + primeTableSize];
	if (marked == nullptr)
	{
	...
	int16_t *primes = reinterpret_cast<int16_t*>(marked + markTableSize);
	int pCnt = generatePrimes(primes, sqrtNum + 2);

	int subDivSize = std::stoi(argv[2]);
	int count = size;
...
#define asmv asm __volatile
	...
	{
		int l = lVal + (offset << 1);
		int r = std::min(lVal + ((offset + subDivSize) << 1) - 2, rVal);
	...
	if (procId == 0)
	{
		...
		std::cout << "There are " << globalCount << " primes less than or equal to " << maxNum << std::endl;
		std::cout << "SIEVE (" << procNum << ") " << std::fixed << tElapsed << std::endl;
	}
